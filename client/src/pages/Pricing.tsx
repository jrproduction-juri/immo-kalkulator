...
const PAID_PLANS: Array<{
...
}> = [
  {
    ...
    features: [
      ...
      { text: "Buy & Hold Szenario (Langfristige Vermietung)", included: true },
  ...
  },
  {
    ...
    features: [
      ...
      { text: "Fix & Flip Szenario (Kaufen, sanieren & verkaufen)", included: true },
      { text: "Buy & Hold mit Exit (Verkauf nach 10 Jahren)", included: true },
      { text: "Eigennutzungs-Strategie (steuerfrei nach § 23 EStG)", included: true },
...
export default function Pricing() {
  ...
  return (
    <div className="min-h-screen" >
      ...
      {/* ── Navbar ─────────────────────────────────────────────────── */}
      <nav className="sticky top-0 z-50 border-b" style={{ backgroundColor: "var(--background)", backdropFilter: "blur(16px)", borderColor: "var(--border)" }}>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5 items-start">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <div className="p-6">
              <p className="text-xs font-semibold uppercase tracking-wider mb-1 text-muted-foreground">Free</p>
              <p className="font-bold text-4xl text-gray-900">0 €</p>
              <p className="text-xs text-muted-foreground mt-1">{FREE_PLAN.billing}</p>
                ...
                {FREE_PLAN.features.map((f) => (
                  <li key={f.text} className="flex items-center gap-2.5 text-sm text-gray-600">
                    {f.included ? (
                      ...
                      <X className="w-4 h-4 shrink-0 text-foreground/80" />
                    )}
          ...
          {PAID_PLANS.map((plan) => {
            ...
            return (
              <div
                ...
                style={plan.highlight ? { background: "linear-gradient(160deg, #0A2540 0%, #1565C0 100%)" } : { background: "var(--card)", border: "1px solid var(--border)" }}
              >
                {/* Badge */}
                {plan.badge && (
                ...
                <div className={`p-6 ${plan.badge ? "" : ""}`}>
                  {/* Plan-Name */}
                  <p className={`text-xs font-semibold uppercase tracking-wider mb-1 ${plan.highlight ? "text-blue-300" : "text-muted-foreground/70"}`}>
                    {plan.name}
                  </p>

                  {/* ── Billing-Toggle ────────────────────────── */}
                  <div className={`flex gap-1 p-1 rounded-lg mb-4 ${plan.highlight ? "bg-muted" : "bg-muted/50"}`}>
                  ...
                  {/* Preis */}
                  <p className={`font-bold text-3xl leading-none ${plan.highlight ? "text-foreground" : "text-foreground"}`}>
                    {getDisplayPrice(plan, currentBilling)}
                  </p>
                  <p className={`text-xs mt-1.5 mb-1 ${plan.highlight ? "text-blue-200" : "text-muted-foreground/70"}`}>
                  ...
                  <p className={`text-sm mb-5 ${plan.highlight ? "text-foreground/70" : "text-gray-500"}`}>
                    {plan.desc}
                  </p>
        ...
        <div className="mt-10 max-w-2xl mx-auto rounded-2xl p-6" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
          <h3 className="font-semibold text-foreground mb-4 text-center">Wie unterscheiden sich die Zahlungsoptionen?</h3>
          <div className="grid grid-cols-3 gap-4 text-center text-sm">
            <div className="p-3 rounded-xl" style={{ background: "rgba(59, 130, 246, 0.1)", border: "1px solid rgba(59, 130, 246, 0.2)" }}>
              ...
              <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>Günstigster Einstieg · volle Flexibilität · jederzeit kündbar</p>
            </div>
            <div className="p-3 rounded-xl" style={{ background: "rgba(16, 185, 129, 0.1)", border: "1px solid rgba(16, 185, 129, 0.2)" }}>
              ...
              <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>~2 Monate geschenkt · günstiger pro Monat als monatlich</p>
            </div>
            <div className="p-3 rounded-xl" style={{ background: "rgba(245, 158, 11, 0.1)", border: "1px solid rgba(245, 158, 11, 0.2)" }}>
              ...
              <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>Einmaliger Höchstpreis · lebenslanger Zugang · keine Folgekosten</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 max-w-2xl mx-auto">
          <h2 className="font-bold text-2xl text-foreground text-center mb-8">Häufige Fragen</h2>
          <div className="space-y-4">
            {[
              ...
              <div key={q} className="rounded-xl p-5" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      ...
      {/* ── Footer ── */}
      <footer className="py-8 mt-8" style={{ backgroundColor: "var(--background)", borderTop: "1px solid var(--border)" }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            />
            <span className="font-semibold text-sm text-foreground">ImmoRenditeTool</span>
          </div>
          <p className="text-xs text-center" style={{ color: "var(--muted-foreground)" }}>
            Alle Berechnungen ohne Gewähr · Keine Anlageberatung · © {new Date().getFullYear()} ImmoRenditeTool
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            <a href="/" className="text-xs" style={{ color: "var(--muted-foreground)" }}>Startseite</a>
            <a href="/dashboard" className="text-xs" style={{ color: "var(--muted-foreground)" }}>Dashboard</a>
            <a href="/impressum" className="text-xs" style={{ color: "var(--muted-foreground)" }}>Impressum</a>
            <a href="/datenschutz" className="text-xs" style={{ color: "var(--muted-foreground)" }}>Datenschutz</a>
            <a href="/agb" className="text-xs" style={{ color: "var(--muted-foreground)" }}>AGB</a>
          </div>
        </div>
      </footer>
    </div>
