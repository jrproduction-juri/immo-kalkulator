...
import {
  ...
  Zap, Calculator, TrendingUp, Save, ArrowLeft, Check, Table, Lock, Star
} from 'lucide-react';
import {
  Dialog,
  DialogContent,
...
import { InvestmentBewertungPanel } from '@/components/InvestmentBewertung';
import { ImmoChatBox, DealData } from '@/components/ImmoChatBox';
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b" style={{ backgroundColor: "var(--muted)", borderColor: "var(--border)" }}>
        <div className="container py-2 flex items-center gap-2">
          <button onClick={() => navigate('/dashboard')} className="flex items-center gap-1.5 text-xs transition-colors" style={{ color: 'var(--muted-foreground)' }}>
            ...
            {!freeResults ? (
                ...
                <div className="px-6 py-8 text-center">
                  ...
                  <div className="grid grid-cols-2 gap-3 max-w-sm mx-auto">
                    {[
                      ...
                      <div key={label} className="p-3 rounded-xl text-left" style={{ background: 'var(--muted)' }}>
              ...
              <div className="rounded-2xl overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  <div className="px-4 pt-4 overflow-x-auto scrollbar-none" style={{ borderBottom: '1px solid var(--border)' }}>
                    <TabsList className="h-9 w-max min-w-full flex-nowrap" style={{ background: 'var(--muted)' }}>
                      ...
                      {isPro && proResults && (
                        <TabsTrigger value="bewertung" className="text-xs">
                  ...
                  <div className="p-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                    ...
                    {isPro && proResults && (
                      <TabsContent value="bewertung" className="mt-0">
                        <InvestmentBewertungPanel bewertung={proResults.investmentBewertung} />
                      </TabsContent>
                    )}
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                {[
                  ...
                  <button
                    key={label}
                    onClick={() => setShowUpgradeModal(true)}
                    className="p-3 rounded-xl transition-all text-left group" style={{ background: 'var(--muted)', border: '1px dashed var(--border)' }}
                  >
                    <div className="flex items-center gap-1.5 mb-1">
                      <Icon className="w-3.5 h-3.5" style={{ color: 'var(--muted-foreground)' }} />
                      <span className="text-xs px-1.5 py-0.5 rounded font-bold btn-gold">Pro</span>
                    </div>
                    <p className="text-xs font-semibold" style={{ color: 'var(--muted-foreground)' }}>{label}</p>
                    <p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>{desc}</p>
                  </button>
      ...
      {/* KI-Chat – schwebt über allem, kennt aktuelle Deal-Daten */}
      <ImmoChatBox
        dealData={freeResults && lastFormData ? ({
          kaufpreis: lastFormData.kaufpreis,
          kaltmiete: lastFormData.kaltmiete,
          wohnflaeche: lastFormData.wohnflaeche,
          eigenkapital: lastFormData.eigenkapital,
          kaufnebenkosten: freeResults.kaufnebenkosten,
          kreditrate: lastFormData.kreditrate,
          zinssatz: lastFormData.zinssatz,
          tilgung: lastFormData.tilgung,
          hausgeld: lastFormData.hausgeld,
          ruecklagen: lastFormData.ruecklagen,
          steuersatz: lastFormData.persönlicherSteuersatz,
          baujahr: lastFormData.baujahr,
          standort: lastFormData.standort,
          bruttomietrendite: freeResults.bruttomietrendite,
          nettomietrendite: proResults?.nettomietrendite,
          cashflowMonatlich: freeResults.nettoCashflowMonat,
          cashflowJaehrlich: freeResults.nettoCashflowJahr,
          eigenkapitalrendite: proResults?.eigenkapitalrendite ?? null,
          gesamtinvestition: freeResults.gesamtinvestition,
          risikoScore: proResults?.risikoBewertung?.gesamtPunkte,
          risikoLabel: proResults?.risikoBewertung?.gesamt,
          investmentScore: proResults?.investmentBewertung?.gesamtScore,
          investmentLabel: proResults?.investmentBewertung?.ampelText,
        } satisfies DealData) : undefined}
      />
